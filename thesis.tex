...
Matrix derivatives are carried out using the numerator-layout convention. For a matrix $X = \mathbb{R}^{p \times q}$ and and $y$ is a scalar, then
...
\begin{equation}
	\text{err}_\text{int}^{I, \Delta t, \ell} = \sup_{ i = 1,\dots,N_u,\ j = 1,\dots, N_v } \left| \partial_v X_{\Delta t, u}^{I, \ell} - \mathcal{D}_u X_{\Delta t, v}^{I, \ell}.
 \right|
\end{equation}
